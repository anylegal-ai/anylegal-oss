...
# Safe XML parser used for every user-supplied XML in this module.
# DOCX/PPTX/XLSX files are uploaded by users (and emitted by an LLM agent) —
# both are untrusted from the validator's point of view. resolve_entities=False
# blocks XXE; no_network=True blocks SSRF via external DTDs; huge_tree=False
# blocks billion-laughs / DoS expansions.
_SAFE_PARSER = lxml.etree.XMLParser(
...
def _safe_parse(source):
    """lxml.etree.parse with the entity/network/size protections enabled."""
    return lxml.etree.parse(source, parser=_SAFE_PARSER)


def _safe_zip_extract(zip_ref, dest_dir):
    """zipfile.ZipFile.extractall with zip-slip protection.

    Resolves every member's destination through Path.resolve() and rejects
    anything that escapes dest_dir. Without this, a malicious DOCX/PPTX can
    ship a member whose name is ../../../etc/passwd or an absolute path and
    overwrite host files when the validator extracts it.
    """
    dest = Path(dest_dir).resolve()
    safe_members = []
    for name in zip_ref.namelist():
        if name.startswith(("/", "\\")) or (len(name) > 1 and name[1] == ":"):
            raise ValueError(f"zip member with absolute path rejected: {name!r}")
        target = (dest / name).resolve()
        try:
            target.relative_to(dest)
        except ValueError as exc:
            raise ValueError(
                f"zip member escapes extraction dir: {name!r} -> {target!r}"
            ) from exc
        safe_members.append(name)
    zip_ref.extractall(dest, members=safe_members)
    ...
    def _get_original_file_errors(self, xml_file):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with zipfile.ZipFile(self.original_file, "r") as zip_ref:
                _safe_zip_extract(zip_ref, temp_path)
